...
export default class extends help{
    ...
    async sendCommandsEmbed(message: Message, msg: Message, category: categorysInterface){
        const commandsEmbed = new MessageEmbed({
            title: "Comandos:",
            description: `${this.client.filteredCommands.filter(c => c.category === category.name).map(command => `\`${this.client.prefix + command.name}\``)}`,
            color: message.member.displayColor
        })
    ...
    createCollector(message: Message, msg: Message, emojis: Array<string>){
        return msg.createReactionCollector((reaction, user) => user.id === message.author.id && emojis.includes(reaction.emoji.name))
    }
        ...
        if(!command) return { error: this.client.errors.help.invalidCommandName }
            ...
            const commandHelpEmbed = new MessageEmbed({
                ...
                fields: [
                    {
                        name: "Descrição:",
                        value: `\`\`\`${command.description ? command.description : "Sem descrição aparente!"}\`\`\``
                    },
                    {
                        name: "Outros nomes:",
                    ...
                    {
                        name: "Como usar:",
                        value: `\`\`\`${command.usage.length > 0 ? command.usage.join("\n") : "Sem exemplo definido!"}\`\`\``
                    },
                    {
                        name: "Categoria:",
